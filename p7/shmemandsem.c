...
int main (void) {
  ...
  for(int i=0;i<10;i++){
    for (int j = 0; j < 10; j++){
         p1m1[i][j]=buffer[2][contador_padre1];
         contador_padre1++;
    }
  }
  ...
  for(int i=0;i<10;i++){
    for (int j = 0; j < 10; j++){
         p1m2[i][j]=buffer[0][contador_padre2];
         contador_padre2++;
    }
  }
...
/*Algunos metodos para el uso de los semaforos*/
void error(char* errorInfo) {
