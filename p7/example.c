  ...
  if((semid=semget(llave,nsems,semban))==-1){
    ...
    if(pid==0){
      ...
      while(i<3){
        ...
        for(j=0;j<nsops;j++){
          printf("\ntsops[%d].sem_num= %d,",j,sops[j].sem_num);
        ...
        if((j=semop(semid,sops,nsops))==-1){
          ...
          printf("\n\nProceso hijo toma control del semaforo: %d/3 veces\n",i+1);
          sleep(5);
          nsops=1;
          sops[0].sem_num=0;
          sops[0].sem_op=-1;
          ...
          if(j=semop(semid,sops,nsops)==-1){
            perror("error en la operacion del semaforo.\n");
          }
          else
      ...
      i=0;
        while(i<3){
          ...
          if((j=semop(semid,sops,nsops))==-1){
            ...
            printf("\tsemop: regreso de semop() %d\n",j);
            printf("\n\nProceso padre toma control del semaforo: %d/3 veces\n",i+1);
            sleep(5);
            nsops=1;
            sops[0].sem_num=0;
            sops[0].sem_op=-1;
